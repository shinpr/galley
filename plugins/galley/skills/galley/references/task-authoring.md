...
- Repository executor defaults come from `environment.yaml`. Read its executor block and resolve CLI, model, and effort independently from the task, then `environment.yaml`. When neither layer sets CLI, use the built-in `claude`. When neither layer sets model, use the selected provider CLI's model default; when neither sets effort, use its reasoning-effort default. Present each effective value with its source. Confirm that any non-empty effort is valid for the effective CLI before approval; model names remain CLI-owned. Record a task override only when the user requests one.
...
Then propose execution settings with user-facing explanations and choices. Treat `loop_budget: 10` as the ordinary-task baseline. Set `timeout_ms` to at least `3600000` (60 minutes) so a productive executor can complete the attempt without being interrupted.
...
- Effective executor: present CLI, model, and effort with `task`, `environment`, `built-in` (CLI only), or `CLI default` as the source of each value. Show a model or effort omitted by both the task and environment as `CLI default`. Validate any non-empty effort against the effective CLI before approval. GLM and Kimi require `glm_api_key` or `kimi_api_key`; Grok requires its authenticated CLI.
...
- `executor.cli` / `model` / `effort`: write only values the user explicitly pins and preserve existing pins. Omitted task fields inherit the current environment. When the environment also omits CLI, Galley uses `cli: claude`. When it also omits model, the selected provider CLI chooses the model; when it also omits effort, the provider chooses the reasoning effort. Use exact user-provided model names. Galley owns prompt transport, so task YAML contains only these three executor fields.
- `execution_policy.timeout_ms`: write the approved per-attempt timeout in milliseconds.
