...
| --- | --- | --- | --- |
| Task authoring | User wants a Galley task YAML, ACs, scope, queueing, or task repair. | `references/task-authoring.md`, `references/authoring-quality.md`; use `scripts/create_task_skeleton.py`; read `references/task.schema.json` before adding a field or collection item whose shape is absent from the generated skeleton, or when repairing validation shape errors. | The task gives the executor one complete outcome contract, uses existing defaults, and stops at the stage requested by the user. |
