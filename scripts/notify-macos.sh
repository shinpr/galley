...
#     command: "/absolute/path/to/notify-macos.sh"
#
...
# osascript as argv values (with-substituted into the AppleScript via a quoted
# argument), NOT concatenated into a shell command. Task summaries are untrusted
# and may contain shell metacharacters; keeping them as argv data prevents
# injection.
set -eu
...
# osascript receives title/body as argv ($1,$2), referenced via `system attribute`
# so the values are data and are never parsed as AppleScript or shell source.
