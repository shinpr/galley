...
- AFK task worktrees are now created from the environment profile's `pr.base` ref. When the source repository has an `origin` remote, Galley first runs a best-effort `git fetch origin <pr.base>` so a stale local `origin/<pr.base>` cannot anchor the new branch behind the remote tip, then resolves `refs/remotes/origin/<base>` (with `refs/heads/<base>` as the origin-less fallback) instead of inheriting the source repository's current HEAD.
