...
- Repository executor defaults also come from `environment.yaml`: when `executor.default_cli` is set, confirm that the task will run with that backend and ask for an override only when the user wants a different model/backend. When no executor default is set, ask for the implementation executor (`claude`, `codex`, or `glm`); if the user leaves the executor unset, new task authoring resolves to Claude.
- Ask for the review supervisor (`claude`, `codex`, or `glm`) only when no verified daemon is running or when the user wants to stop/restart the daemon before queueing. A running verified daemon's supervisor is current state, not a task setting.
...
- Executor backend (`executor.cli`): include it as one Task YAML execution setting. Use `environment.yaml` `executor.default_cli` when present; if it is absent, ask for `claude`, `codex`, or `glm` and use Claude when the user leaves the setting unset. `glm` runs the Claude Code binary against GLM's Anthropic-compatible endpoint (Z.ai); it requires `glm_api_key` in `daemon.yaml`, accepts the same `executor.effort` values as Claude, and applies to every executor role (setup, acceptance skeleton, and implementation). When `glm` is selected but `glm_api_key` is unset or unknown, resolve it before queueing: ask the user to set it in `daemon.yaml`, choose `claude`/`codex` instead, or queue without starting the daemon. Explain that `executor.cli` controls both the implementation attempt and, when enabled, acceptance skeleton preflight. The selected backend, `executor.model`, and `executor.effort` also apply to the built-in skeleton creator. It is independent from the daemon supervisor.
...
- Supervisor: present the current daemon supervisor when a verified daemon is running. When no verified daemon is running, present the planned supervisor; the daemon default is `claude`, and `codex` or `glm` can be selected instead (`glm` needs `glm_api_key` in `daemon.yaml`). The supervisor is the acceptance gate, so present the choice and let the user decide. The daemon supervisor controls only review; it is independent from `executor.cli`, which drives both the implementation attempt and acceptance skeleton preflight.
...
The skeleton script resolves `executor.cli` from the explicit `--executor-cli` argument first, then `environment.yaml` `executor.default_cli`, then Claude when no environment setting exists.
...
- `executor.cli`: use the resolved environment executor default when `environment.yaml` has `executor.default_cli`; otherwise use Claude when no environment default or explicit user choice exists. Preserve an explicit `executor.cli` already present in existing task YAML. This is a task YAML setting and is separate from the daemon supervisor.
...
- `executor.effort`: set the approved effort for the selected executor. Claude and `glm` accept `low`, `medium`, `high`, `xhigh`, or `max`; Codex accepts `minimal`, `low`, `medium`, `high`, `xhigh`, or `max`. Use `high` for ordinary implementation work unless the user asks for lower cost or higher effort. A specific model may reject a provider-valid effort it does not support; that surfaces as an upstream Codex error, not a Galley validation error.
