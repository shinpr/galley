...
- Repository executor defaults come from `environment.yaml`. Read its executor block, resolve CLI, model, and effort independently using task override, then environment value; the built-in fallback covers only CLI (`claude`), while an omitted effort or model is left to the selected provider CLI. Present each effective value with its source. Confirm that any explicit effort is valid for the effective CLI before approval; model names remain CLI-owned. Record a task override only when the user requests one.
...
- Effective executor: present CLI, model, and effort with `task`, `environment`, `built-in` (CLI only), or `CLI default` as the source of each value. Show an omitted model or effort as `CLI default`. Validate any explicit effort against the effective CLI before approval. GLM requires `glm_api_key`; Grok requires its authenticated CLI.
...
- `executor.cli` / `model` / `effort`: write only values the user explicitly pins and preserve existing pins. Omitted fields inherit the current environment; an omitted CLI then falls back to `cli: claude`, while an omitted effort or model lets the selected provider CLI choose its own reasoning effort and model. Use exact user-provided model names. Galley owns prompt transport, so task YAML contains only these three executor fields.
