...
- `required_checks.shell_path`: optional explicit executable path for the configured `required_checks.shell` kind. Use this for non-standard Bash installs (MSYS2, Cygwin, portable Git outside `C:\Program Files\Git`), custom PowerShell installs, or intentionally WSL-based setups where Galley's auto-discovery is not the right choice. The value is used verbatim for required-check execution and discovery is skipped. Setting `shell_path` without a concrete `required_checks.shell` (or with `required_checks.shell: auto`) is a profile validation error because there is no shell kind to associate the override with.
