...
- Executor backend (`executor.cli`): include it as one Task YAML execution setting. Use `environment.yaml` `executor.default_cli` when present; if it is absent, ask for `claude` or `codex` and use Codex when the user leaves the setting unset. Explain that `executor.cli` controls both the implementation attempt and, when enabled, acceptance skeleton preflight. The selected backend, `executor.model`, and `executor.effort` also apply to the built-in skeleton creator. It is independent from the daemon supervisor.
