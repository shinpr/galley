...
- Supervisor: present the current daemon supervisor when a verified daemon is running. When no verified daemon is running, present the planned supervisor; the daemon default is `codex`, and `claude` can be selected for Claude review.
