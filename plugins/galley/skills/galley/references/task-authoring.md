...
- `executor.effort`: set the approved effort for the selected executor. Claude and `glm` accept `low`, `medium`, `high`, `xhigh`, or `max`; Codex accepts `low`, `medium`, or `high`. Use `high` for ordinary implementation work unless the user asks for lower cost or higher Claude-only effort.
