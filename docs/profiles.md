...
- `pr.base`: base branch for opened PRs. The same value is also used as the start-point ref when Galley creates a brand-new task worktree for an AFK run, so a queued task no longer inherits whatever ref the source repository's HEAD happened to point at when the daemon claimed it. Before resolving the start-point, when the source repository has an `origin` remote the daemon runs a best-effort `git fetch origin <base>` so a stale local `refs/remotes/origin/<base>` cannot anchor the new branch behind the remote tip; fetch failures (offline, missing on remote) are non-fatal. The daemon then resolves `refs/remotes/origin/<base>` first (matching the `gh pr create --base <base>` intent for AFK runs that ultimately push to origin) and falls back to `refs/heads/<base>` so origin-less local checkouts and the smoke test keep working; if `pr.base` is non-empty and neither ref exists in the source repository, the daemon fails the claimed task with a descriptive error. When `pr.base` is empty the new branch falls back to the source repository's current HEAD, matching the previous behavior. Reused worktree paths and existing branches keep their current tip — the start point only applies when `git worktree add -b <branch>` is materializing the branch for the first time.
