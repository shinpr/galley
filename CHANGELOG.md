...
- AFK task worktrees are now created from the environment profile's `pr.base` ref (`refs/remotes/origin/<base>` first, falling back to `refs/heads/<base>`) instead of inheriting the source repository's current HEAD, so queued tasks branch from the intended PR base even when the daemon's checkout is sitting on an unrelated commit.
