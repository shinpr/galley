...
- Optional `preflight.acceptance_skeleton` task YAML section. When `enabled: true`, the daemon runs an `AcceptanceSkeletonPreflight` stage after `inputfiles.Prepare` and before the first executor attempt, persists `runs/<run-id>/preflight_result.json` as the runtime source of truth, adds a skeleton obligations section to the executor work order, and runs a daemon-side acceptance gate before `acceptSupervisorVerdict` finalizes the task. Accepted verdicts with missing or failed required skeleton/checkpoint evidence are downgraded to `needs_supervisor_review` with a user-visible reason. The section is default-disabled; existing tasks without it run unchanged. A `preflight.acceptance_skeleton.creator.command` runs inside the worktree to materialize skeleton files and emit a JSON manifest (`GALLEY_SKELETON_MANIFEST`/`GALLEY_SKELETON_ACS`/`GALLEY_SKELETON_ALLOWED_PATHS` are exported); without a creator the statically declared `outputs[]` are materialized (auto-materialization of missing files is the fallback path only). For creator manifests, absolute paths and parent-directory traversal are rejected before the allowed-path check (so an `allowed_paths` of `"."` means "inside the prepared worktree only"), and the stage fails if a creator-declared output file does not already exist on disk rather than auto-creating it. `galley task show` now lists runtime skeleton outputs in text output. The required-check acceptance gate treats a required quality check's `preferred_commands` as an ordered fallback list (mirroring `result.Complete`): a check is satisfied when any preferred command has passing evidence, failed when none passed but one has a failed entry, and missing only when no executor result recorded evidence for any of its commands.
