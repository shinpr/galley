...
- **Acceptance skeleton preflight**: optional `preflight.acceptance_skeleton` runs before the first executor attempt to materialize AC-linked test skeletons in the worktree. With `creator.command` set, the daemon runs that command inside the worktree (exporting `GALLEY_SKELETON_MANIFEST`, `GALLEY_SKELETON_ACS`, `GALLEY_SKELETON_ALLOWED_PATHS`); the command writes the skeleton files and a JSON manifest of outputs. Without `creator.command`, the statically declared `outputs[]` are materialized instead. Galley validates returned paths/AC IDs against `preflight.acceptance_skeleton.allowed_paths` (default `scope.allowed_paths`), persists `runs/<run-id>/preflight_result.json`, runs each `checkpoint_command` through the result-completion path after every attempt, feeds the evidence to the executor work order and supervisor, and downgrades an accepted verdict to `needs_supervisor_review` when a required skeleton or required check is missing or failed. The section is default-disabled. See [docs/task-yaml.md](docs/task-yaml.md).
