...
- `required_checks.shell`: optional shell for Galley-owned `quality.required_checks` execution. Values are `auto`, `sh`, `bash`, `cmd`, `powershell`, and `pwsh`. When unset or `auto`, macOS/Linux use `/bin/sh`; Windows prefers a standard Git for Windows Bash install (`C:\Program Files\Git\bin\bash.exe`, `C:\Program Files\Git\usr\bin\bash.exe`, the `(x86)` equivalents, or Git Bash inferred from a discoverable `git.exe`) and falls back to `cmd.exe`. Any other PATH-discovered `bash.exe` — including the WSL launcher (`C:\Windows\System32\bash.exe`), a `WindowsApps` shim, MSYS2, Cygwin, Scoop, or Chocolatey-managed Bashes — is not auto-selected; use `required_checks.shell` plus `required_checks.shell_path` to opt into those installs. Galley records the resolved shell in verification evidence. Use `bash` for checks that rely on POSIX tools or syntax such as `grep`, `find`, `xargs`, `test`, `$()`, or single-quoted shell strings. Use `cmd`, `powershell`, or `pwsh` when the profile commands are intentionally Windows-native.
- `required_checks.shell_path`: optional explicit executable path for the configured `required_checks.shell` kind. Use this for non-standard Bash installs (MSYS2, Cygwin, portable Git outside `C:\Program Files\Git`), custom PowerShell installs, intentionally WSL-based setups, or Unix environments that need a pinned shell such as Nix or Homebrew Bash. The value is used verbatim for required-check execution and discovery is skipped; Galley does not infer whether the executable name matches the selected shell kind, so operators are responsible for pairing `shell` with a compatible executable. Setting `shell_path` without a concrete `required_checks.shell` (or with `required_checks.shell: auto`) is a profile validation error because there is no shell kind to associate the override with.
