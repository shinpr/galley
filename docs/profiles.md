...
- `required_checks.shell`: optional shell for Galley-owned `quality.required_checks` execution. Values are `auto`, `sh`, `bash`, `cmd`, `powershell`, and `pwsh`. When unset or `auto`, macOS/Linux use `/bin/sh`; Windows uses standard Git for Windows Bash when discoverable and otherwise falls back to `cmd.exe`. On Windows, explicit `bash` also uses Git for Windows discovery; if no standard Git Bash is found, Galley fails and asks for `shell_path`.
- `required_checks.shell_path`: optional executable path override for required-check shell selection. Recognized shell executable names (`bash`, `sh`, `cmd`, `powershell`, `pwsh`, with optional `.exe`) can stand alone and Galley infers the invocation style from the filename. When both `shell` and `shell_path` are set, `shell_path` takes precedence; `shell` is fallback metadata only when the executable filename is not recognized. Galley records both the resolved shell kind and launched executable path in verification evidence.
...
- `pr.comments.enabled`: poll PR comments and accept any comment whose trimmed body starts with `/galley`. The free-form prefix `/galley <request>` is treated as the request. Mid-line mentions or `/galley` lines that are not the first non-whitespace token of the comment are ignored. Trust boundary: a `/galley` command is accepted only when the comment author's login matches the PR author login recorded on the task (`pr.author_login`, persisted at PR creation time). Comments that fail this check are marked processed without requeueing; when `pr.comments.reply` is enabled, Galley posts a concise rejection reply. Task files without a recorded `pr.author_login` fail closed.
...
- `setup.commands[]`: optional ordered setup plan used by the setup executor before the acceptance skeleton preflight and before the implementation executor. Each entry has a `run` shell command and an optional human-readable `why`. When `setup` is present, Galley passes the listed commands to the setup executor as the prior plan to try, diagnose, and repair in one model context. When `setup` is absent, the setup executor discovers a working plan from `environment.commands`, repository signals, and conventions. On success, the successful plan is atomically written back to this file when it differs, so subsequent tasks reuse it without rediscovery. The setup phase writes `runs/<run-id>/setup_result.json` (attempted commands, readiness evidence, source, repair guidance) and, when a learned plan is persisted, `runs/<run-id>/environment_update.json` (profile rewrite audit record). See `examples/environment-local.yaml` for a worked example.
