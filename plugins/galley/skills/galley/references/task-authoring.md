...
- Task YAML settings: edit authority, retry budget, per-attempt timeout, AC test skeleton preflight, and blocking severity policy.
...
- `preflight.acceptance_skeleton.enabled`: set the approved value. The generated skeleton emits `preflight.acceptance_skeleton.enabled: false` by default so the runtime gate is visible; flip it to `true` when you approved the preflight stage. When enabled, write the initial preflight config only and leave runtime `outputs[]` to the skeleton creator.
