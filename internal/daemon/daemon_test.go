...
func TestRunOnceRecordsExecutorIdleTimeoutEvidence(t *testing.T) {
	root := filepath.Join(t.TempDir(), ".agent-workflow")
	repo := initDaemonGitRepo(t)
	promptPath, schemaPath := writeDaemonPromptFiles(t)
	// Executor emits initial output, then stops producing any stdout/stderr.
	claudeBin := writeFakeClaude(t, "printf 'starting work\\n'\nsleep 30\n")
	taskPath := filepath.Join(root, "tasks", "queued", "task.yaml")
	if err := os.MkdirAll(filepath.Dir(taskPath), 0o755); err != nil {
		t.Fatal(err)
	}
	writeDaemonTask(t, taskPath, repo)
	setTimeoutMS(t, taskPath, 10000)
	...
	if len(failedTask.Attempts) == 0 {
		t.Fatal("expected an attempt recording the executor idle timeout")
	}
	last := failedTask.Attempts[len(failedTask.Attempts)-1]
	if last.ClaudeStatus != "idle_timed_out" {
		t.Fatalf("claude status got %q, want idle_timed_out", last.ClaudeStatus)
	}
	if last.Error == nil || last.Error.Phase != "executor" || last.Error.Kind != "idle_timeout" {
		t.Fatalf("attempt error got %#v", last.Error)
	}
	if !strings.Contains(last.Error.Message, "idle timeout") {
		t.Fatalf("attempt error message got %q", last.Error.Message)
	}
	runResults, err := filepath.Glob(filepath.Join(root, "runs", "*", "attempt-1", "run_result.json"))
	if err != nil || len(runResults) != 1 {
		t.Fatalf("run_result.json glob got %v err=%v", runResults, err)
	}
	data, err := os.ReadFile(runResults[0])
	if err != nil {
		t.Fatal(err)
	}
	...
	if !runResult.IdleTimedOut {
		t.Fatalf("run_result.json idle_timed_out got false: %s", data)
	}
	if runResult.TimedOut {
		t.Fatalf("idle timeout must be distinct from total timeout: %s", data)
	}
}

func TestRunOnceRecordsCodexSupervisorIdleTimeoutEvidence(t *testing.T) {
	root := filepath.Join(t.TempDir(), ".agent-workflow")
	repo := initDaemonGitRepo(t)
	promptPath, schemaPath := writeDaemonPromptFiles(t)
	claudeBin := writeFakeClaude(t, "echo change > daemon-output.txt\necho '{\"status\":\"completed\",\"summary\":\"done\",\"files_modified\":[\"daemon-output.txt\"],\"acceptance_criteria\":[{\"id\":\"AC1\",\"status\":\"satisfied\",\"evidence\":[\"diff\"],\"notes\":\"done\"}],\"verification\":[],\"decisions\":[],\"risks\":[]}'\n")
	// Codex supervisor emits one event, then stalls without writing a verdict.
	codexBin := writeFakeCommand(t, "codex", "cat >/dev/null\nprintf '{\"event\":\"thinking\"}\\n'\nsleep 30\n")
	taskPath := filepath.Join(root, "tasks", "queued", "task.yaml")
	if err := os.MkdirAll(filepath.Dir(taskPath), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if err == nil {
		t.Fatal("expected codex supervisor idle timeout")
	}
	failedTask, err := task.Load(filepath.Join(root, "tasks", "failed", "task.yaml"))
	if err != nil {
		t.Fatal(err)
	...
	if len(failedTask.Attempts) == 0 {
		t.Fatal("expected an attempt recording the supervisor idle timeout")
	}
	last := failedTask.Attempts[len(failedTask.Attempts)-1]
	if last.SupervisorVerdict != "idle_timeout" {
		t.Fatalf("supervisor verdict got %q, want idle_timeout", last.SupervisorVerdict)
	}
	if last.Error == nil || last.Error.Phase != "supervisor" || last.Error.Kind != "idle_timeout" {
		t.Fatalf("attempt error got %#v", last.Error)
	}
	if !strings.Contains(last.Error.Message, "codex supervisor failed") || !strings.Contains(last.Error.Message, "idle timeout") {
		t.Fatalf("attempt error message got %q", last.Error.Message)
	}
}

func TestRunOnceRecordsClaudeSupervisorIdleTimeoutEvidence(t *testing.T) {
	root := filepath.Join(t.TempDir(), ".agent-workflow")
	repo := initDaemonGitRepo(t)
	promptPath, schemaPath := writeDaemonPromptFiles(t)
	// One binary serves both roles: as executor it completes with a diff; as the
	// built-in claude supervisor (GALLEY_CLAUDE_GUARD_MODE=supervisor) it stalls.
	claudeBin := writeFakeCommand(t, "claude", `if [ "$GALLEY_CLAUDE_GUARD_MODE" = "supervisor" ]; then
  cat >/dev/null
  printf '{"status":'
  sleep 30
fi
...
`)
	...
	if err == nil {
		t.Fatal("expected claude supervisor idle timeout")
	}
	failedTask, err := task.Load(filepath.Join(root, "tasks", "failed", "task.yaml"))
	if err != nil {
		t.Fatal(err)
	...
	if !strings.Contains(last.Error.Message, "claude supervisor failed") || !strings.Contains(last.Error.Message, "idle timeout") {
		t.Fatalf("attempt error message got %q", last.Error.Message)
	}
}
